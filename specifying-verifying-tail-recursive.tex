...
% Use XeLaTeX to compile this document.
...
\newunicodechar{ʳ}{\ensuremath{^{\text{r}}}}
\newunicodechar{ˡ}{\ensuremath{^{\text{l}}}}
\newunicodechar{≟}{\ensuremath{\overset{\text{?}}{=}}}

% This document requires the following font in order to compile
% correctly all the unicode characters used in the document.
\setmonofont{FiraCode-VF.ttf}
...
The base case is trivial, because both sides of the equality in the resulting type reduce 
to the same term, which is:
\begin{align*}
  \MVerb{len []} &= \MVerb{0} & \text{(by definition)} \\
  \MVerb{len-tl [] 0} &= \MVerb{0}
\end{align*}

Therefore, we can fill the first hole in our proof with the \Verb{refl} constructor, such
...
\begin{align*}
  ...
  \MVerb{len-tl (x∷xs) 0} &= \MVerb{len-tl xs (suc 0)} \\
                                   &= \MVerb{len-tl xs 1} \\
\end{align*}

We need to prove that \Verb{suc (len xs)≡len-tl xs 1}. This time, we cannot
...
The type of \Verb{ind-h} is \Verb{len xs≡len-tl xs 0}. The left sides
...
\Verb{n + len-tl xs 0} on the left side and \Verb{len-tl xs n} on the
...
\begin{align*}
  \MVerb{r <> reduce-tl [] empty} &= \MVerb{r <> empty} \\
  \MVerb{reduce-tl [] (empty <> r)} &= \MVerb{empty <> r} \\
\end{align*}
...
\begin{align*}
  \MVerb{r <}&\MVerb{> reduce-tl (x∷xs) empty} \\
    &= \MVerb{r <> reduce-tl xs (empty <> f x)} \\
  \MVerb{red}&\MVerb{uce-tl (x∷xs) (empty <> r)} \\
    &= \MVerb{reduce-tl xs ((empty <> r) <> f x)} \\
\end{align*}
...
\begin{align*}
  \MVerb{r <}&\MVerb{> reduce-tl (x∷xs) s} \\
    &= \MVerb{r <> reduce-tl xs (s <> f x)} \\
  \MVerb{red}&\MVerb{uce-tl (x∷xs) (s <> r)} \\
    &= \MVerb{reduce-tl xs ((r <> s) <> f x)} \\
\end{align*}
...
tail-recursive counterpart has been presented, along with an Agda module to automatically
generate the functions and the proof from an arbitrary monoid. As far as we know there is
no related work in the literature which gives a formal proof of the extensional equality
and transformation of a recursive function into a tail-recursive one, which is the main
contribution of this work. 

The tail-recursive function generally improves the time complexity of the original
recursive function and opens the possibility of performing tail-call optimization by the
compiler, leading to a more space efficient function execution
